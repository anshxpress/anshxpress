<div align="center">

<table border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="center" valign="middle">
      <h1>Hi there, I'm Anshul Gupta</h1>
    </td>
    <td align="center" valign="middle">
      <img src="https://user-images.githubusercontent.com/74038190/216654128-ad1c5827-e18e-43a6-974b-3669cbb082b9.gif" width="60" />
    </td>
  </tr>
</table>

### 🎯 Data Analytics Enthusiast | Power BI · SQL · Python

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/anshxpress019)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:anshulgupta19122001@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/anshxpress)

*"The best insights come from messy data — cleaned with code, visualized with purpose, and delivered with clarity."*

<img src="https://komarev.com/ghpvc/?username=anshxpress&color=blueviolet&style=for-the-badge&label=PROFILE+VIEWS" alt="Profile Views" />

</div>

---

## 🧠 About Me

I am a passionate and detail-oriented **Data Enthusiast**. I specialize in transforming raw, complex datasets into visual stories that drive business growth.

* 🔭 **Current Focus:** Advanced Exploratory Data Analysis (EDA) and Dashboarding.
* 🎓 **Expertise:** Strong foundation in SQL, Power BI, and Python-based data analysis.
* 💬 **Ask me about:** Data cleaning pipelines, interactive storytelling, and sentiment analysis.
* ⚡ **Goal:** Seeking Data Analyst roles and exciting freelance collaborations.

---

## 🛠️ Tech Toolbox

### 💻 Programming & Querying
![Python](https://skillicons.dev/icons?i=python) ![MySQL](https://skillicons.dev/icons?i=mysql) ![Java](https://skillicons.dev/icons?i=java) ![C++](https://skillicons.dev/icons?i=cpp) ![Bash](https://skillicons.dev/icons?i=bash)

### 📊 Data Analysis & Visualization
<p>
  <img src="https://img.shields.io/badge/PowerBI-F2C811?style=for-the-badge&logo=Power-BI&logoColor=black" alt="Power BI" />
  <img src="https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=Tableau&logoColor=white" alt="Tableau" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" alt="Pandas" />
  <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white" alt="NumPy" />
  <img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white" alt="Streamlit" />
  <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" alt="Excel" />
</p>

### ☁️ Cloud & Tools
![Azure](https://skillicons.dev/icons?i=azure) ![Git](https://skillicons.dev/icons?i=git) ![GitHub](https://skillicons.dev/icons?i=github) ![VSCode](https://skillicons.dev/icons?i=vscode) ![Jupyter](https://skillicons.dev/icons?i=jupyter)

---

## 🚀 Key Projects

### 📈 Customer Feedback Analysis
* **Overview:** Performed sentiment analysis on globally collected movie reviews.
* **Impact:** Classified reviews into Positive, Negative, or Neutral sentiments using NLP.
* **Tools:** `Python`, `Pandas`, `NLTK`.

### 📊 E-Commerce Data Dashboard
* **Overview:** End-to-end EDA on transaction-level data with an interactive dashboard.
* **Impact:** Analyzed user behavior, sales performance, and product trends.
* **Tools:** `Excel`, `Power BI`, `SQL`.

### 🌐 India Demographic Insights
* **Overview:** Interactive dashboard visualizing India's demographic indicators.
* **Impact:** Mapped literacy rates, population distribution, and sex ratio data.
* **Tools:** `Python`, `Seaborn`, `Streamlit`.

---

## 📈 GitHub Stats

<div align="center">
  <!-- <img src="https://github-readme-stats.vercel.app/api?username=anshxpress&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=anshxpress&layout=compact&theme=tokyonight&hide_border=true" width="40%" /> -->
  <br/>
  <img src="https://streak-stats.demolab.com?user=anshxpress&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

---

## 📫 Let's Connect

<div align="center">

📧 [anshulgupta19122001@gmail.com](mailto:anshulgupta19122001@gmail.com) | 💼 [linkedin.com/in/anshxpress019](https://linkedin.com/in/anshxpress019)

**Open to Data Analyst roles, freelance projects, and collaborations!**

</div>
